...
from metrices import check_caps, cost
from dotenv import load_dotenv
from metrices import new_stats, record, row
from obs import save_run
...
def run(task : str , iteration = max_iterations, system : str = SYSTEM , trace: list | None = None, stats: dict | None = None):
    ...
    with tracer.start_as_current_span("run") as sp:
        sp.set_attribute("loop","ota")
        
        for turn in range (1, iteration+1):
            
            check_caps(stats)

            with tracer.start_as_current_span(f"turn {turn}") as turn_span:
                ...
                record(stats, usage, MODEL, len(msg.tool_calls or []))

                turn_span.set_attribute("gen_ai.request.model", MODEL)
                turn_span.set_attribute("gen_ai.usage.input_tokens", usage.prompt_tokens)
                turn_span.set_attribute("gen_ai.usage.output_tokens", usage.completion_tokens)
                turn_span.set_attribute("app.usd", cost(usage, MODEL))
                turn_span.set_attribute("app.finish_reason", choice.finish_reason)
                ...
                match choice.finish_reason:
                    case "tool_calls":
                        for call in calls:
                            with tracer.start_as_current_span("tool") as tool_span:
                                tool_span.set_attribute("tool.name", call.function.name)
                                try:
...
if __name__ == "__main__":
    ...
    save_run("OTA", TASK, stats, trace, answer, ok)